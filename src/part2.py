...
class cybertruck2:
    latest_img = None
    ackermann_msg_id = 0
    # dir1='~/catkin_ws/src/cybertruck2/src' 
    # os.chdir(dir1)
    def __init__(self):
        # receive
        #global cur_vel
    ...
    def im_callback(self, data):
        ...
        '''
        img = cv_image[:,:,::-1]
       # cv2.imwrite('yellow_bgr.png', img)
    ...
    def detect_flag(self, img):
        ...
        print('Unique Mask2: ', np.unique(mask2))
        ...
        # commenting testing code here
        #cv2.imwrite('binary.png',mask)
        #box = cv2.boxPoints(cv2.minAreaRect(max_contour))
        #box = np.int0(box)
        #img_bgr = cv2.cvtColor(img, cv2.COLOR_HSV2BGR)
        #draw_box = cv2.drawContours(img_bgr,[box],0,255,2)
        #cv2.imwrite("draw_box.png", draw_box)
        ...
        # show the images
        #cv2.imwrite("draw_box.png", np.hstack([img_bgr, output]))
        ...
        if(len(contours) > 0):
            ...
            # box_width, box_height = cv2.minAreaRect(contours[0])[1]
        ...
        #self.if_flag = output[0]
        #self.x = output[1]
        #self.flag_reached = output[2]
        # print('test_if_flag', output[0])
        # print('test_flag_reached', output[2])
        return output
    ...
    def search_env(self):
        if(self.if_flag == False):
          ...
          #  print("The flag has reached")
        #    a = [0.0, 0.0, 0.0]
        #    return a

        # a = [steering, gas, brake(useless here)]
        a = [0.0, 0.05, 0.0]
        center = 640
        # best kp = 0.05
        kp = 0.0006
        ...
        #print("error: ", goal - cur)
        des_a = kp*(goal-cur) 
        #kd*(goal_vel-cur_vel)
        cl_a = np.clip(des_a, -max_a, max_a)
        a[0] = cl_a
        #print("print a when True: ", a[0])
...
def register_flag(img):
    ...
    # box = np.int0(box)
    # draw_box = cv2.drawContours(img,[box],0,255,1)
    # cv2.imwrite('register.png', draw_box)
    f = open('flag2.txt', 'w')
    output = str(pixel[0]) + ','+  str(pixel[1]) + ',' + str(pixel[2])
    print('output', output)
    f.write(output)
...
if __name__ == '__main__':
    ...
    #test_image = cv2.imread('yellow_bgr002.png')

    #print("Shape of test_image", np.shape(test_image))

    #register_flag(test_image)

    node = cybertruck2()
    rospy.spin()
    # image_message = latest_img
    # bridge = CvBridge()
    # cv_image = bridge.imgmsg_to_cv2(image_message, desired_encoding = 'passthrough')
    


    # print(detect_flag(cv_image))
    # listener()
    
    #while not rospy.is_shutdown():
        # steer = search_env(self.if_flag, self.x, cur_vel)
    #    publish_ctrl(rp_ctrls, ctrl)
    #    r.sleep()
